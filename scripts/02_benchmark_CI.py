...
for i, row in df.iterrows():
    ...
    df.loc[i, 'run'] = str(counter+22) # 22 instead of 6 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

# save DataFrame df
#df.to_csv("metadata/paper/benchmark_paper_exp6.csv", index=True)
df = pd.read_csv("metadata/paper/benchmark_paper_exp7.csv", index_col=0, keep_default_na=False)
...
for index in range(len(cols)):
    ...
    for baseline in [True,False]: # [True, False]
        if (model_type in ["grf","grfo"]) and baseline == True:
