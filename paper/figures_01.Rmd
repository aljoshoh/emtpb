...
EMT_gs <- readRDS("metadata/EMT_gs.rds")
...
ggplot(model_eval_sum) + 
  ...
  theme_classic() + xlab("observed IC50") + ylab("predicted IC50") + ggtitle("Luminespib") +
...
ggplot(predicted_ic50s2_sum) + 
  stat_smooth(aes(x = 10**IC50.1_Nutlin_3a.1047.GDSC1, y = 10**mean_base), color = "gray45", fill = NA,
              method = "lm") + 
  geom_point(aes(x = 10**IC50.1_Nutlin_3a.1047.GDSC1, y = 10**mean_base), color = "gray45") +
  stat_smooth(aes(x = 10**IC50.1_Nutlin_3a.1047.GDSC1, y = 10**mean_emt), color = "darkgreen", fill = NA,
              method = "lm") +
  geom_point(aes(x = 10**IC50.1_Nutlin_3a.1047.GDSC1, y = 10**mean_emt), color = "darkgreen") + 
  theme_classic() + xlab("observed IC50") + ylab("predicted IC50") + ggtitle("Nutlin-3a") +
  #theme(plot.title = element_text(size = 36),
  #      axis.title = element_text(size = 34),
  #      axis.text = element_text(size = 15)) +
  annotate(geom = "text", x = 10**0.8, y = 10**4.2, color = "gray45",hjust = 0,
           label = paste("r =", round(cor.test(predicted_ic50s2_sum$IC50.1_Nutlin_3a.1047.GDSC1, 
                                                        predicted_ic50s2_sum$mean_base)$estimate, digits = 2))) +
  annotate(geom = "text", x = 10**0.8, y = 10**3.7, color = "darkgreen", hjust = 0,
           label = paste("r =", round(cor.test(predicted_ic50s2_sum$IC50.1_Nutlin_3a.1047.GDSC1, 
                                                        predicted_ic50s2_sum$mean_emt)$estimate, digits = 2))) +
   scale_x_log10(
   breaks = scales::breaks_log(n = 5),
   labels = scales::trans_format("log10", scales::math_format(10^.x))
 ) +
...
ggplot(GDSC_dr_wide2[GDSC_dr_wide2$`TCGA Desc` == "SKCM",], aes(x=EMT_score, y = exp(`328-GDSC1`))) + 
  ...
  xlab("EMT score") + ylab("log10(IC50 [µM])") + ggtitle("SNX 2112", subtitle = "") +
  theme_classic()+ #+ theme(plot.title = element_text(size = 20), 
...
data <- skcm_emt[skcm_emt$NQO1 == "high" & skcm_emt$genotype_CT != "-",]
cortest <- cor.test(data$EMT_by_cancer, data$Tanespimycin.1026.GDSC1)
ggplot(data, aes(x=EMT_by_cancer, y = 10**Tanespimycin.1026.GDSC1)) + 
  geom_smooth(method = "lm", color = "gray50", fill = "gray65") + geom_point() +
  xlab("EMT score") + ylab("log10(IC50 [µM]) 17-AAG") + ggtitle("17-AAG response", subtitle = "functional NQO1 expression") +
  theme_classic() + #+ theme(plot.title = element_text(size = 20),
                  #        plot.subtitle = element_text(size = 18, face = "italic"),
                  #        axis.title = element_text(size = 20),
                  #        axis.text = element_text(size = 15),
  ...
  annotate("text", label = paste0("r = ",as.numeric(round(cortest$estimate, 2)),"\n","p = ",round(cortest$p.value, 3)) , 
           x = max(data$EMT_by_cancer), y = 0.00000001, vjust = 0, hjust = 1)
...
ggplot(data, aes(x=EMT_score, y = exp(`1559-GDSC2`), color = EMT_score)) + 
     ...
     geom_point() +
     xlab("EMT score") + ylab("log10(IC50 [µM])") + ggtitle("Luminespib in melanoma (GDSC)") +
     ...
     theme_classic() + theme(#plot.title = element_text(size = 20), 
         ...
         #        axis.text = element_text(size = 15),
         legend.position = "none") +
     scale_color_gradient(low = "navyblue", high = "orange", name = "EMT score")+
     scale_y_log10(
         breaks = scales::trans_breaks("log10", function(x) 10^x),
         labels = scales::trans_format("log10", scales::math_format(10^.x)),
         limits = c(0.008,20)
...
ggplot(EMTscores[EMTscores$`TCGA Desc` %in% c("BRCA", "COREAD", "OV", "SKCM","LUAD"),], 
...
performances[which] <- readRDS(paste0(path,"paper/metadata/",which,"_performances_v2.rds")) %>%
  dplyr::filter(fraction_extrapolated < 0.7 & !is.na(pvalue)) # filter NAs and extrapolated
}; performances <- do.call(rbind, performances)

#use new performances
#d <- readRDS("ml_output/ML_output_df.Rdata")
keep <- readRDS("../cancer_spec_keep_drugs_alex.RData")
d <- performances
####
d <- left_join(keep, d, by = c("TCGA","Drug"))
d <- na.omit(d)

d$Drug = unlist(lapply(d$Drug, function(x) strsplit(x,"\\.")[[1]][1]))
d <- d %>%
  #group_by(TCGA) %>%
  mutate(pval_adj = p.adjust(pval, method = "BH"))
sort(d$pval_adj) %>% head

# filter
#d <- d[d$Drug %in% drugs_sum$d,]

h0 <- sort(unlist(na.omit(d[,"pval"])))[max(which(sort(unlist(na.omit(d[,"pval_adj"])))<0.05))]
ggplot(data = d)+
  geom_point(data = d[(d$TCGA %in% c("BRCA", "COREAD", "OV", "SKCM","LUAD")),], aes(x = cor_emt-cor_mut, y = -log10(pval), color = TCGA), alpha = 0.8)+
  geom_point(data = d[!(d$TCGA %in% c("BRCA", "COREAD", "OV", "SKCM","LUAD") & (d$pval_adj < 0.05)),], aes(x = cor_emt-cor_mut, y = -log10(pval)), color = "grey", alpha = 0.8)+
  geom_point(data = d[(d$TCGA %in% c("BRCA", "COREAD", "OV", "SKCM","LUAD") & (d$pval_adj < 0.05)),], aes(x = cor_emt-cor_mut, y = -log10(pval), color = TCGA), alpha = 0.8)+
  geom_text_repel(data = d[ d$TCGA %in% c("BRCA", "COREAD", "OV", "SKCM","LUAD") & (d$pval_adj < 0.05),], #  (d$Drug %in% c("Luminespib","BX795","Teniposide")) &
                  aes(x = cor_emt-cor_mut, y = -log10(pval), label = Drug), color = "black", size = 4, show_guide = F, min.segment.length = 0.02)+
  theme_classic()+
  ...
  annotate("text", x = 0, y = -log10(h0), label = "FDR<5%", hjust = 0, vjust = 0)+
