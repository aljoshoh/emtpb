...
```{r causal_effect_skcm_auc, fig.width=3, fig.height=3.5}
x <- "id"
y <- "effectsize_grf"
...
```{r causal_effect_skcm_ic50, fig.width=2, fig.height=3.5}
x <- "id"
y <- "effectsize_grf"
...
```{r brca_subtypes, fig.width=4, fig.height=4}
...
summary(lm(data = brca_emt[brca_emt$drug == "AZD7762 (2)",],formula =  `1022-GDSC2` ~ BRCA1_mut + BRCA2_mut + ESR1 + PAM50 + mak))
stat.test.anova <- brca_emt[brca_emt$drug == "AZD7762 (2)",] %>%
    ...
    adjust_pvalue(method = "none") %>%
    add_significance() 
ggplot((brca_emt%>%dplyr::select(-drug))[(brca_emt$PAM50 == "Basal")&(brca_emt$`BRCA1/2 mutation` == 0),]%>%distinct, aes(x = mak, y = `1022-GDSC2`)) +
    geom_point()+
    theme_minimal()+
    theme(legend.position = "none") + xlab("BRCA subtype") + ylab("log(IC50")+
    #stat_pvalue_manual(stat.test, bracket.nudge.y = 0.4)+
    labs(
        subtitle = get_test_label(stat.test.anova, detailed = TRUE))+
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), plot.margin = margin(t = 0, r = 0, b = 0, l = 50, unit = "pt"))
