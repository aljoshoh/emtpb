...
validation_aucs <- "metadata/goeksu/exp1/AUCs.xlsx"
validation_ic50s <- "metadata/goeksu/exp1/IC50s.xlsx"
...
```{r ablation_correlation, fig.width=3, fig.height=3}
...
ggplot(q) + 
  stat_smooth(aes(x = truth.x, y = preds.x), color = "gray45", fill = NA,
              method = "lm") + 
  geom_point(aes(x = truth.x, y = preds.x), color = "gray45") +
  stat_smooth(aes(x = truth.x, y = preds.y), color = "darkgreen", fill = NA,
              method = "lm") +
  geom_point(aes(x = truth.x, y = preds.y), color = "darkgreen") + 
  theme_minimal() + 
  ...
  annotate(geom = "text", x = 0.5, y = 0.9, color = "darkgreen", hjust = 0,
           label = paste("r =", round(cor.test(q$truth.x, q$preds.y)$estimate, digits = 2)))
```

```{r volcano_skcm_auc_mak, fig.width=6, fig.height=3}
...
```{r causal_effect_skcm, fig.width=3, fig.height=5}
x <- "id"
y <- "effectsize_grf"
...
p <- ggplot(data = df_4)+
    ...
    xlab("")+
    ylab("Standardised estimated causal effect")
p
```
...
ggp <- ggpairs(emtp, columns = 2:ncol(emtp), aes(colour=`TCGA Desc`),
...
```{r emt_correlations_label, fig.width=2, fig.height=2}
print(ggp[5,5] + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)))
```

```{r emt_correlations_summary, fig.width=2, fig.height=2}
print(ggp[5,1]+scale_y_continuous(breaks = c(1,5,9)))
print(ggp[5,2]+scale_y_continuous(breaks = c(1,5,9)))
print(ggp[5,3]+scale_y_continuous(breaks = c(1,5,9)))
print(ggp[5,4]+scale_y_continuous(breaks = c(1,5,9)))
```

```{r mitf, fig.width=5, fig.height=5}
dat_skcm <- emtp %>% 
...
dat_skcm <- left_join(dat_skcm,resp_auc[,colnames(resp_auc) %in% c(sig_drugs,"COSMIC ID")], by = "COSMIC ID")
                      
skcm_emt <- dat_skcm %>%
  mutate(Luminespib = case_when(`1559-GDSC2` < median(na.omit(`1559-GDSC2`)) ~ "sensitive", T ~ "resistant")) %>%
  mutate(Staurosporine = case_when(`1034-GDSC2` < median(na.omit(`1034-GDSC2`)) ~ "sensitive", T ~ "resistant")) %>%
  mutate(`CHIR-99021` = case_when(`1241-GDSC1` < median(na.omit(`1241-GDSC1`)) ~ "sensitive", T ~ "resistant")) %>%
  #mutate(mak = case_when(mak < median(na.omit(mak)) ~ "epithelial", T ~ "mesenchymal")) %>%
  dplyr::select(c("COSMIC ID","mak","gsva","tan","secrier","MITF","Luminespib","Staurosporine","CHIR-99021")) %>%
  
  dplyr::rename(drug = name, resp = value) %>%
  #pivot_longer(cols = c("mak","gsva","tan","secrier"))
  dplyr::select(-c("mak","gsva","tan","secrier")) %>% distinct
...
ggboxplot(skcm_emt, x = "resp", y = "MITF", color = "resp", add = "jitter",facet.by = "drug")+theme_minimal()+
  scale_color_manual(values = c("resistant" = "darkolivegreen4", "sensitive" = "darkolivegreen2"))+
...
```{r tf_res}
enr <- readRDS(paste0(path,"metadata/paper/tf_target_enrichments.rds"))
enr_red <- enr %>% filter(sign == "positive") %>% group_by(drug,cancertype) %>% slice_min(Adjusted.P.value)
...
```{r tf_heatmap, fig.width= 4, fig.height=7}
limma_luminespib <- readRDS(paste0(path,"metadata/paper/tf_diff_genes_luminespib.rds"))
...
skcm_emt <- left_join(skcm_emt, resp_auc[,c("COSMIC ID","1559-GDSC2"),drop = F])
...
```{r causal_effect, fig.width=4.5, fig.height=5}
x <- "id"
y <- "effectsize_grfo"
df_4 <- (df_3[df_3$resp_type == "auc",])
df_4 <- df_4[(df_4$`c(strat)` %in% label_data$`c(strat)`),]
...
```{r luad_rock, fig.width=3.5, fig.height=3.5}
# GSK269962A
df_4 <- df_3
df_4 <- df_4[(df_4$`c(strat)` %in% label_data$`c(strat)`),]
...
luad_emt <- dat %>%
  #mutate(mak = case_when(mak < median(na.omit(mak)) ~ "epithelial", T ~ "mesenchymal")) %>%
  mutate(GSK269962A = case_when(`1192-GDSC1` < median(na.omit(`1192-GDSC1`)) ~ "sensitive", T ~ "resistant")) %>%
  dplyr::select(c("COSMIC ID","mak","gsva","tan","secrier","1192-GDSC1")) %>%
  pivot_longer(cols = c("1192-GDSC1")) %>%
  dplyr::rename(drug = name, resp = value)
  #pivot_longer(cols = c("mak","gsva","tan","secrier"))
  #dplyr::select(-c("gsva","tan","secrier")) %>% distinct

data <- luad_emt
cortest <- cor.test(data$mak, data$resp)
ggplot(data, aes(x=mak, y = data$resp)) + 
  geom_smooth(method = "lm", color = "gray50", fill = "gray65") + geom_point() +
  xlab("EMT score") + ylab("AUC GSK269962A") + ggtitle("GSK269962A response") +
  ...
  annotate("text", label = paste0("r = ",as.numeric(round(cortest$estimate, 2)),"\n","p = ",round(cortest$p.value, 6)) , 
           x = min(na.omit(data$mak)), y = 0.05, vjust = 1, hjust = 0)
```

```{r brca_subtypes, fig.width=5, fig.height=5}
#brca subtype50s Jaaks 2023
...
brca_emt <- dat %>%
  mutate(`AZD7762 (1)` = case_when(`1022-GDSC2` < median(na.omit(`1022-GDSC2`)) ~ "sensitive", T ~ "resistant")) %>%
  mutate(`AZD7762 (2)` = case_when(`1402-GDSC1` < median(na.omit(`1402-GDSC1`)) ~ "sensitive", T ~ "resistant")) %>%
  #mutate(mak = case_when(mak < median(na.omit(mak)) ~ "epithelial", T ~ "mesenchymal")) %>%
  dplyr::select(c("COSMIC ID","mak","gsva","tan","secrier","ESR1","AZD7762 (1)","AZD7762 (2)","PAM50","1022-GDSC2","1402-GDSC1")) %>%
...
ggplot(data, aes(x=EMT_score, y = data$`1026-GDSC1`)) + 
  geom_smooth(method = "lm", color = "gray50", fill = "gray65") + geom_point() +
  xlab("EMT score") + ylab("AUC 17-AAG") + ggtitle("17-AAG response", subtitle = "functional NQO1 expression") +
  theme_minimal() + #+ theme(plot.title = element_text(size = 20),
                  #        plot.subtitle = element_text(size = 18, face = "italic"),
                  #        axis.title = element_text(size = 20),
                  #        axis.text = element_text(size = 15),
...
```{r snx2112, fig.width=3.5, fig.height=3.5}
data <- EMTscores %>% dplyr::filter(`TCGA Desc` == "SKCM")
data <- left_join(data, resp_auc)
cortest <- cor.test(data$EMT_score, data$`328-GDSC1`) #Tanespimycin.1026.GDSC1
ggplot(data, aes(x=EMT_score, y = data$`328-GDSC1`)) + 
  geom_smooth(method = "lm", color = "gray50", fill = "gray65") + geom_point() +
  xlab("EMT score") + ylab("AUC SNX 2112") + ggtitle("SNX 2112 response") +
  theme_minimal() + #+ theme(plot.title = element_text(size = 20),
                  #        plot.subtitle = element_text(size = 18, face = "italic"),
                  #        axis.title = element_text(size = 20),
                  #        axis.text = element_text(size = 15),
...
```{r onalespib, fig.width=3.5, fig.height=3.5}
...
# 50134: tanespimycin
# 362338: SNX-2112
dr_onalespib <- drug_response_ccle[drug_response_ccle$master_cpd_id %in% c(688229,50134,362338),]
...
for(drugname in na.omit(unique(dr_onalespib$cpd_name))){
  ...
  cortest
  p <- ggplot(ccle_skcm_dr, aes(x=EMT_score, y=AUC)) + 
...
```{r luminespib_experiment, fig.width=4, fig.height=4}
...
orderedcelllines <- c("SK-MEL-5","IGR-37","RPMI-7951") # put here what cells to highlight (A-375,SK-MEL1)
...
ggplot(data, aes(x=EMT_score, y = exp(`1559-GDSC2`), color = EMT_score)) + 
     ...
     geom_text_repel(data = data %>% dplyr::filter(EMT_score<1.5), aes(label = CellLineName), min.segment.length = 0.1, box.padding = 0, force =1, nudge_x = -0.15,nudge_y = 1, hjust = 1, color = "black") +
     theme_minimal() + theme(#plot.title = element_text(size = 20), 
         #        axis.title = element_text(size = 20),
         #        axis.text = element_text(size = 15),
         legend.position = "none") +
...
```{r validation_summary_auc, fig.height=1.5, fig.width=5}
# plot confidence band for luminespib AUC
...
```{r validation_summary_ic50, fig.height=1.5, fig.width=5}
# plot confidence band for luminespib AUC
