  ...
  )
{
  ...
  plist[["auc"]] <- ggplot(data, aes(x=EMT_score, y = !!sym(dr))) + 
  ...
  plist[["ic50"]] <- ggplot(data, aes(x=EMT_score, y = exp(!!sym(dr)))) + 
    geom_smooth(method = "lm", color = "gray50", fill = "gray65") + geom_point() +
...
ggplot(q) + 
  stat_smooth(aes(x = exp(truth.x), y = exp(preds.x)), color = "gray45", fill = NA,
              method = "lm") + 
  geom_point(aes(x = exp(truth.x), y = exp(preds.x)), color = "gray45") +
  stat_smooth(aes(x = exp(truth.y), y = exp(preds.y)), color = "darkgreen", fill = NA,
              method = "lm") +
  geom_point(aes(x = exp(truth.y), y = exp(preds.y)), color = "darkgreen") + 
  theme_minimal() + 
  ...
  annotate(geom = "text", x = 3, y = 1.0, color = "gray50",hjust = 0,
           label = paste("r =", round(cor.test(q$truth.x, q$preds.x)$estimate, digits = 2))) +
  annotate(geom = "text", x = 3, y = 1.4, color = "darkgreen", hjust = 0,
           label = paste("r =", round(cor.test(q$truth.x, q$preds.y)$estimate, digits = 2)))+
  scale_x_log10(
...
```{r causal_effect_skcm_auc, fig.width=3, fig.height=5}
x <- "id"
y <- "effectsize_grf"
...
```{r volcano_skcm_ic50_mak, fig.width=6, fig.height=3}
# Figure 1 volcano for cancer types and drugs (SKCM)
min_hits <- 1
pastel_colors <- brewer.pal(6, "Pastel2")
df_3 <- df %>%
...
```{r causal_effect_skcm_ic50, fig.width=2, fig.height=5}
x <- "id"
y <- "effectsize_grf"
...
```{r causal_effect_auc, fig.width=4.0, fig.height=5}
x <- "id"
y <- "effectsize_grfo"
df_4 <- (df_3[df_3$resp_type == "auc",])
df_4 <- df_4[(df_4$`c(strat)` %in% label_data$`c(strat)`),]
...
```{r causal_effect_ic50, fig.width=3.5, fig.height=5}
x <- "id"
y <- "effectsize_grfo"
df_4 <- (df_3[df_3$resp_type == "ic50",])
df_4 <- df_4[(df_4$`c(strat)` %in% label_data$`c(strat)`),]
...
luad_emt <- dat %>%
  ...
  dplyr::select(c("COSMIC ID","mak","gsva","tan","tagliazucchi","1192-GDSC1")) %>%
...
brca_emt <- dat %>%
  ...
  #mutate(mak = case_when(mak < median(na.omit(mak)) ~ "epithelial", T ~ "mesenchymal")) %>%
  dplyr::select(c("COSMIC ID","mak","gsva","tan","tagliazucchi","ESR1","AZD7762 (1)","AZD7762 (2)","PAM50","1022-GDSC2","1402-GDSC1")) %>%
...
cortest <- cor.test(data$EMT_score, unlist(data[,dr]))
ggplot(data, aes(x=EMT_score, y = !!sym(dr))) + 
  geom_smooth(method = "lm", color = "gray50", fill = "gray65") + geom_point() +
...
data <- exp[exp$NQO1_binary == "high",] # & exp$genotype_CT != "-"
cortest <- cor.test(data$EMT_score, unlist(data[,dr])) #Tanespimycin.1026.GDSC1
ggplot(data, aes(x=EMT_score, y = exp(!!sym(dr)))) + 
  geom_smooth(method = "lm", color = "gray50", fill = "gray65") + geom_point() +
  xlab("EMT score") + ylab("IC50 17-AAG [µM]") + ggtitle("17-AAG response")+ #, subtitle = "functional NQO1 expression") +
  theme_minimal() + #+ theme(plot.title = element_text(size = 20),
                  #        plot.subtitle = element_text(size = 18, face = "italic"),
                  #        axis.title = element_text(size = 20),
                  #        axis.text = element_text(size = 15),
...
```{r onalespib_auc, fig.width=2, fig.height=2}
...
```{r onalespib_ic50, fig.width=2, fig.height=2}
...
for(drugname in na.omit(unique(dr_onalespib$cpd_name))){
  ...
  cortest
  p <- ggplot(ccle_skcm_dr, aes(x=EMT_score, y=apparent_ec50_umol)) + 
    stat_smooth(method = "lm", color = "gray50", fill = "gray65") + 
    geom_point() + 
    ggtitle(paste0(drugname," in CCLE"))+ theme_minimal()+
...
Vredci <- full_join(Vred %>% 
  ...
  mutate(name = factor(name,levels = c("RPMI-7951","IGR-37","SK-MEL-5","null")))
...
```
# change name from secrier to tagliazucchi
```
